...
def main():
    ...
    parser.add_argument(
        "--codec", default="h264", choices=["h264", "av1", "libx264"], help="Video codec: h264 (h264_nvenc), av1 (av1_nvenc), libx264 (CPU)"
    )
    ...
    input_video = ffmpeg.input("pipe:", format="rawvideo", pix_fmt="bgr24", s=f"{w}x{h}", r=fps, thread_queue_size=512)
    ...
    output_stream = ffmpeg.output(*inputs, args.output, **output_args).overwrite_output()
